...
def self_attention(inputs, da, r, batch_size, name="self_attention", reuse=None):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        att = tf.nn.softmax(
            tf.reshape(tanh_Ws1_time_X_and_time_Ws2,
        ...
        return output_vec, att


def bidaf(passage, query, batch_size, name="BiDAF_match", reuse=None):
    with tf.variable_scope(name, reuse=reuse):
        ...
        p2q_att = tf.matmul(tf.reshape(mul, shape=[-1, seq_len]), W)
        # p2q_att.shape = [batch_size, seq_len, seq_len]
        p2q_att = tf.nn.softmax(
            tf.reshape(p2q_att, shape=[batch_size, seq_len, seq_len]),
            dim=-1,
        ...
        q2p_att = tf.nn.softmax(
            tf.reshape(q2p_att, shape=[batch_size, seq_len, seq_len]),
            dim=-1,
        ...
        # concat outputs
        # concat_outputs.shape = [batch_size, seq_len, context_dim*4]
        concat_outputs = tf.concat(
            [passage, query, passage2query, query2passage], -1)
...
def mlp(mlp_inputs, mlp_hidden_size, num_class, keep_prob,
        name='multi_layer_perceptron', reuse=tf.AUTO_REUSE):

    with tf.variable_scope(name, reuse=reuse):
        # shape(mlp_inputs) == [batch_size, per_input_size]
        # print("mlp_inputs.shape == ", mlp_inputs.shape.as_list())
        _, per_input_size = mlp_inputs.shape.as_list()
        ...
        weight = {
            'h1': tf.get_variable(
                name="weight_h1",
                shape=[per_input_size, mlp_hidden_size],
...
if __name__ == '__main__':
    input_seqs = tf.random_normal(shape=[4, 7, 5], dtype=tf.float32, stddev=1, seed=1)
    #outputs_hw = highway(input_seqs)
    outputs_vec, att = self_attention(
        input_seqs, 13, 1, 4, name='self_att', reuse=tf.AUTO_REUSE)
